...
class TabuList {
	public:
		TabuList(const uint32_t& numberOfActivities, const uint32_t& maxIter);

		bool isPossibleMove(const uint32_t& i, const uint32_t& j, const MoveType& type) const;
		void addTurnToTabuList(const uint32_t& i, const uint32_t& j, const MoveType& type);
		void bestSolutionFound();
		size_t goToNextIter();

		void randomizeTabuList();
